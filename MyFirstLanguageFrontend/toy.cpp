...
static std::unique_ptr<ExprAST> ParseForExpr() {
    ...
    if (CurTok != tok_identifier)
        return LogError("expected idendifier after for");
...
static std::unique_ptr<PrototypeAST> ParsePrototype() {
    ...
    if (Kind && ArgNames.size() != Kind)
        return LogErrorP("Invalid number of oprands for operator");
...
void InitializeModuleAndPassManager(void) {
    ...
    // Create a new pass manager attatched to it
    TheFPM = std::make_unique<legacy::FunctionPassManager>(TheModule.get());

    // Do simple "peephole" optimizations add bit-twiddling optzns.
...
Value *BinaryExprAST::codegen() {
    ...
    // If it wasn't a builtin binary operator, it must be a user defiend one.
    // Emit a call to it.
...
Value *CallExprAST::codegen() {
    ...
    if (!CalleeF)
        return LogErrorV("Unknown function refreenced");

    // If argument mismatch error.
    if (CalleeF->arg_size() != Args.size())
        return LogErrorV("Incorrect # arguments passwd");
...
static void HandleTopLevelExpression() {
    ...
    if (auto FnAST = ParseTopLevelExpr()) {
        if (FnAST->codegen()) {
            ...
            // JIT the module containning the anonymous expression, keeping a handle
